...
const initSocket = (httpServer) => {
    ...
    waitCutNamespace.on('connection', (socket) => {
        ...
        socket.on('request_move_up', async (orderId) => {
            console.log(`⚡️ หลังบ้านได้รับคำสั่งเลื่อนขึ้นของไอดี: ${orderId}`);
            try {
                const updatedQueue = await wait_cut_model_1.WaitCutModel.moveOrderUp(orderId);
        ...
        socket.on('request_move_down', async (orderId) => {
            console.log(`⚡️ หลังบ้านได้รับคำสั่งเลื่อนลงของไอดี: ${orderId}`);
            try {
                const updatedQueue = await wait_cut_model_1.WaitCutModel.moveOrderDown(orderId);
                // 🎯 เปลี่ยนจาก io.emit เป็น waitCutNamespace.emit เพื่อให้เด้งเฉพาะหน้ารอตัด
                waitCutNamespace.emit('update_queue_table', updatedQueue);
            }
            catch (error) {
